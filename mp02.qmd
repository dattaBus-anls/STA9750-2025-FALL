...
![Data Flow Diagram – ACS, BLS, and Derived Metrics](images/data_flow_diagram.jpg){fig-align="center" width="85%"}
...
```{r}
#| code-fold: true
#| code-summary: "🏗️ Click to see the ACS Download Code"
#| echo: true
...
```{r}
#| code-fold: true
#| code-summary: "🏗️ Click to see the code"
#| message: true
...
::: {.callout-note title="📍 Why Use the CBSA Level?"}
...
```{r}
#| code-fold: true
#| code-summary: "🏢 Click to see industry code function (get_bls_industry_codes)"
#| echo: true
#| message: false
...
#| code-fold: true
#| code-summary: "▶ Click to see industry code download call"
#| echo: true
#| message: true
...
```{r}
#| code-fold: true
#| code-summary: "💵 Click to see wage data function (get_bls_qcew_annual_averages)"
#| echo: true
#| message: true
...
#| code-fold: true
#| code-summary: "▶ Click to see wage data download call"
#| echo: true
#| message: true
...
```{r}
#| code-fold: true
#| code-summary: "🧮 Click to see CBSA key helpers"
#| echo: true
...
#| code-fold: true
#| code-summary: "🗺️ Click to see CBSA key creation code"
#| echo: true
...
#| code-fold: true
#| code-summary: "🏷️ Click to see CBSA name attachment code"
#| echo: true
...
```
🪄 **Result:**
This code connects the **building permit data** to the list of **metro area names (CBSAs)** so each permit record shows where it belongs (e.g., “New York–Newark”).
---
  
```{r wages_cbsa_fix, cache=TRUE}
#| code-fold: true
#| code-summary: "💼 Click to see BLS wage data cleaning code"
#| echo: true
...
#| code-fold: true
#| code-summary: "🔗 Click to see dataset joins (integration_check)"
#| echo: true
#| message: false
#| warning: false
...
#| code-fold: true
#| code-summary: "🏘️ Click to see Top 10 CBSA permit summary code"
#| echo: true
...
#| code-fold: true
#| code-summary: "📈 Click to see largest housing-permit spike code"
#| echo: true
...
#| code-fold: true
#| code-summary: "💵 Click to see Top 10 CBSAs by income (2015)"
#| echo: true
...
#| code-fold: true
#| code-summary: "🏦 Click to see Finance & Insurance wage analysis code"
#| echo: true
...
```{r}
#| code-fold: true
#| code-summary: "🔧 Normalize CBSA -> std_cbsa (zero-pad), then sanity-check"
#| message: false
#| warning: false
...
```{r}
#| code-fold: true
#| code-summary: "📊 Click to see the code for Q1 (fixed)"
...
✅**Summary:**
The CBSA shown above had the **largest cumulative number of housing permits** issued between **2010 and 2019**, indicating the region’s sustained construction growth during the decade.
---
...
```{r}
#| code-fold: true
#| code-summary: "📊 Click to see the code for Q2"
...
✅**Summary:**
This year marks **Albuquerque’s construction peak**, reflecting a possible rebound in housing development or infrastructure expansion after economic slowdowns.
---
...
```{r}
#| code-fold: true
#| code-summary: "📊 Click to see the code for Q3"
...
#| cache: false
#| code-fold: true
#| code-summary: "👀 Click to see the code that builds the NYC data-scientist employment table"
...
knitr::kable(
  ...
  caption = "Annual Leader in NAICS 5182 Employment by CBSA (2009–2023, excluding 2020)"
)

# ---- 5) “Last year NYC led” line (optional) ----
...
```{r}
#| code-fold: true
#| code-summary: "📊 Q5 (robust + key padding)"
#| message: false
#| warning: false
...
✅**Summary:**
This shows **the year finance and insurance wages formed the largest share** of NYC’s total payrolls, signaling the sector’s relative economic dominance. 
---
...
```{r}
#| code-fold: true
#| code-summary: "📈 Task 3.1 — Rent vs. Income by CBSA (2009)"
#| message: false
#| warning: false
...
[⬇️ Download this chart (PNG, 300 dpi)](images/rent_income_2009.png)
...
```{r}
#| code-fold: true
#| code-summary: "📈 Task 3.2 — NAICS 62 vs. Total Employment (evolution over time)"
#| message: false
#| warning: false
...
[⬇️ Download this chart (PNG, 300 dpi)](images/naics62_vs_total_emp.png)

📈 **Task 3.3:** The evolution of **average household size over time**, using **different lines to represent different CBSAs**.  

```{r}
#| code-fold: true
#| code-summary: "📈 Q3 — Household Size Over Time (NYC & LA highlighted + national IQR & median)"
#| message: false
#| warning: false
#| fig-width: 9
#| fig-height: 5.5
...
[⬇️ Download this chart (PNG, 300 dpi)](images/household_size_over_time_compelling.png)
...
```{r}
#| code-fold: true
#| code-summary: "🏗 Click to see code for Task: 4"
#| message: false
#| warning: false
...
stop("Task 6 needs objects 'RB_PRIMARY' and 'metrics_std'. Run Tasks 4–5 first.")
}
...
cat("🟦 **Figure 1 – Rent Burden Over Time**  \n",
"Each colored line represents a CBSA’s rent-to-income ratio across time; ",
...
labs(
title    = "Rent Burden Over Time by CBSA (2009–2023, 2020 excluded)",
subtitle = "Colored lines meet ALL four YIMBY criteria • Dashed = national mean (first year)",
...
"Blue points mark metros where higher housing growth coincided with falling rent burden — ",
...
tip = paste0(
NAME, "\nAvg Growth (0–100): ", round(avg_growth, 1),
...
subtitle = "Right of line = above-avg growth • Below line = RB decreased\nBlue = meets all four YIMBY criteria",
x = "Average Housing Growth Index (0–100)",
y = "Change in Rent Burden (Last − First)"
) +
theme_minimal(base_size = 13)
...
dplyr::mutate(
`High Early RB`       = ifelse(crit_high_early_rb, "✅", "❌"),
`RB Decreased`        = ifelse(crit_rb_decreased,  "✅", "❌"),
`Pop Grew`            = ifelse(crit_pop_growth,     "✅", "❌"),
`Above-Avg Growth`    = ifelse(crit_above_avg_growth, "✅", "❌"),
`RB Change`           = scales::percent(rb_change, accuracy = 0.1),
`Avg Growth (0–100)`  = round(avg_growth, 1)
) |>
dplyr::select(
Metro = NAME, `RB Change`, `Avg Growth (0–100)`,
...
`RB Change` = scales::percent(rb_change, accuracy = 0.1),
`Avg Growth (0–100)` = round(avg_growth, 1),
`High Early RB` = "✅", `RB Decreased` = "✅",
`Pop Grew` = "✅", `Above-Avg Growth` = "✅"
)
}
...
cat("## Policy Brief — Pro-Housing Partnership Program (PHPP)\n\n")
...
print(kable(cmp, caption = glue("Latest-Year Snapshot ({LAST_YEAR}) — Rent Burden vs. Housing Growth")))

# cat("### Occupational Footprint: Key Voter Groups\n")

# Occupational table (format Employees with commas)
if (nrow(occ_tbl) == 0) {
  cat(glue("*(Data unavailable for selected metros — fallback to national averages.)*\n\n"))
} else {
  print(
    kable(
      occ_tbl %>% dplyr::mutate(Employees = scales::comma(Employees)),
      caption = glue("Employment by Occupation — {LAST_YEAR}")
    )
  )
}
...
# cat("## 📄 Policy Brief Summary — Pro-Housing Partnership Program\n\n")
...
**Selected Sponsors:**  
- **Primary Sponsor:** `r primary_metro` — a YIMBY success city showing reduced rent burden and strong housing growth.  
- **Co-Sponsor:** `r co_metro` — a NIMBY city struggling with affordability, representing the need for federal incentives.  
<br>
...
**Policy Rationale:**  
PHPP now also considers youth vitality. `r primary_metro` scores **`r ma_primary`** on Millennial Appeal, while `r co_metro` scores **`r ma_co`**, showing how affordability, growth, and generational appeal align under PHPP’s goals.
...
#| code-fold: true
#| code-summary: "📈 Click to see how the highlighted spaghetti plot is built"
#| echo: true
#| message: false
#| warning: false
#| fig-cap: "Household size over time by CBSA (2009–2023; 2020 omitted). NYC & LA highlighted; others faint for context."
...
```{r}
#| code-fold: true
#| code-summary: "👥 Millennial Appeal (ACS 25–34 share + BLS Arts share; cached; 0–100 composite)"
#| echo: true
#| message: false
#| warning: false
#| fig-cap: "Millennial Appeal (ages 25–34 + arts employment) at the CBSA level. Higher index suggests stronger youth appeal."
...
cat(glue::glue('
<details>
<summary><b>🧭 Click to see the Millennial Appeal Summary ({{latest_year}})</b></summary>

**Millennial Appeal Insight (`{latest_year}`):**

The *Millennial Appeal Index (0–100)* combines two signals —
1) the share of residents aged 25–34 (ACS), and  
