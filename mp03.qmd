...
How are trees distributed across NYC’s council districts, where are maintenance needs most acute, and what district-level program would best preserve canopy and public safety?

New York City’s green spaces are a defining feature of its urban landscape from tree-lined streets to expansive parks that shape livability. In this mini-project, I’ll explore the **NYC TreeMap dataset** to visualize and analyze how these natural assets are distributed across boroughs and council districts. I’ll then propose a new **NYC Parks Department program** designed to expand the benefits of trees to all New Yorkers.

![Aerial view of Central Park, NYC — symbolizing the city’s green canopy.](images/central-park.jpg){fig-align="center" width="85%"}
...
-   🧩 Programmatically acquire and process real-world geospatial data using the **NYC Open Data API**\
-   🌳 Combine multiple spatial datasets to understand NYC’s urban forest composition\
-   📊 Create compelling data visualizations using `ggplot2` and `sf`\
-   🏛️ Translate data-driven insights into an actionable **policy proposal** for the Parks Department
...
1.  **Data Ingestion & Cleaning** – Transform raw datasets into standardized R data frames\
2.  **Data Combination & Alignment** – Merge tree points with NYC council-district boundaries\
3.  **Descriptive Analysis** – Compute statistics on tree counts, density, and species diversity\
4.  **Visualization** – Use layered `geom_sf()` maps and graphics to communicate insights\
5.  **Policy Design** – Develop a concise data-supported proposal for improving NYC’s tree coverage

---

## 🌐 Data Acquisition 

This section focuses on collecting and preparing the core spatial data needed for NYC’s tree canopy analysis. Both dataset the **City Council District boundaries** and **the Forestry Tree Points** are retrieved from **official NYC Open Data sources**. They are cleaned, standardized to the WGS84 coordinate system, and stored locally to support reproducible geospatial analysis and visualization.

---

### Task 01: 🗺️ Download NYC City Council Districts Dataset
...
This analysis uses the **official “City Council (Clipped to Shoreline)” shapefile — Latest Release 25C (August 2025)**, downloaded from the NYC Department of City Planning’s [City Council dataset portal](https://www.nyc.gov/content/planning/pages/resources/datasets/city-council) using the ZIP file `nycc_25c.zip`. To respect the data provider’s servers, this script downloads the ZIP file only once and reuses the cached copy in future renders. After extraction, the boundaries are converted from the local NYC projection to **WGS 84 (EPSG 4326)** to ensure alignment with other geospatial layers. If plotting proves slow, I can simplify the polygons using `st_simplify(dTolerance = 5)` without visible loss of detail.
...
#| message: false
#| warning: false
#| code-summary: "🗺️ Click to see the Council Districts download + results"
...
# --- Configuration (replace with the latest ZIP from NYC Planning → 'Clipped to Shoreline') ---
...
download_council_districts <- function(
  ...
  if (!file_ready(zip_path)) {
    say("Downloading council districts (zip)…")
  ...
  if (!is.null(simplify_tolerance_m)) {
    say("Simplifying geometry with dTolerance = %s m…", simplify_tolerance_m)
...
if (!is.na(n_feat) && is_wgs84 && n_feat >= 51) {
  cat(sprintf("✅ Successfully downloaded and transformed the City Council District boundaries to WGS84.\n"))
  cat(sprintf("✅ Detected %d polygon features (expected ≥ 51). Ready for spatial ops & visualization.\n", n_feat))
  cat(sprintf("ℹ️  CRS: %s (EPSG: %s)\n", st_crs(districts)$input %||% "unknown", epsg_val %||% NA_integer_))
} else if (!is.na(n_feat)) {
  if (!is_wgs84) {
    cat("⚠️ Districts loaded but CRS is not WGS84. Please ensure st_transform(..., crs = 'WGS84').\n")
  }
  if (n_feat < 51) {
    cat(sprintf("⚠️ Districts loaded but feature count is low (%d < 51). Verify I am using the 'Clipped to Shoreline' dataset.\n", n_feat))
  }
} else {
  cat("❌ Districts could not be verified. Re-run the download step and check the ZIP URL.\n")
}
...
# Safety guard
if (!exists("districts")) {
  cat("⚠️ Districts object not available yet — run the full document (not just this chunk).\n")
  knitr::knit_exit()
}
...
if (!is.na(n_feat) && is_wgs84 && n_feat >= 51) {
  cat("✅ Successfully downloaded and transformed the City Council District boundaries to WGS84.\n")
  cat(sprintf("✅ Detected %d polygon features (expected ≥ 51). Ready for spatial ops & visualization.\n", n_feat))
  cat(sprintf("ℹ️ CRS: %s (EPSG: %s)\n", sf::st_crs(districts)$input %||% "unknown", epsg_val %||% NA_integer_))
} else if (!is.na(n_feat)) {
  if (!is_wgs84) cat("⚠️ Districts loaded but CRS is not WGS84. Please ensure st_transform(..., crs='WGS84').\n")
  if (n_feat < 51) cat(sprintf("⚠️ Feature count is low (%d < 51). Verify dataset integrity.\n", n_feat))
} else {
  cat("❌ Districts could not be verified. Re-run the download step and check the ZIP URL.\n")
}
...
> ⚙️ *This chunk is cached for speed. I will clear the cache and re-run the entire document before submission to ensure full reproducibility.*

### Task 02:🌳 NYC Tree Points
...
The **Forestry Tree Points** dataset was accessed directly through the NYC Open Data **SODA2 API**. The endpoint URL (`https://data.cityofnewyork.us/resource/hn5i-inap.geojson`) was obtained via the site’s **Export → API Endpoint → GeoJSON (SODA2)** option. All requests are made with the **`httr2`** package (explicitly required by the assignment) rather than `httr` or `RSocrata`. The script paginates through the full dataset using `$limit = 10 000` and incrementing `$offset` until the last page contains fewer rows than the limit—ensuring complete yet polite retrieval. Each page is saved locally under a consistent naming scheme such as `trees_<offset>_<limit>.geojson` and reused on reruns to avoid repeated downloads.
...
#| message: false
#| warning: false
#| code-summary: "🌳 Click to see the Tree Points full-dataset download + results (httr2 + paging)"
suppressPackageStartupMessages({
  library(httr2)
  library(glue)
  library(sf)
...
download_tree_points <- function(
  ...
  say("Starting paginated download from NYC OpenData (GeoJSON, httr2)…")

  repeat {
    target <- file.path(out_dir, glue("trees_{sprintf('%05d', offset)}_{page_limit}.geojson"))

    if (!file_ready(target)) {
      say("Requesting page %d (offset=%d, limit=%d)…", page, offset, page_limit)
...
say("Reading %d GeoJSON page(s)…", length(all_files))
...
if (!identical(geom_type, "POINT")) cat("⚠️ Expected POINT geometries; got: ", geom_type, "\n")
if (st_crs(trees)$epsg != 4326) cat("⚠️ CRS is not EPSG:4326 (WGS84).\n")
trees
...
if (!is.na(n_rows) && n_rows > 0) {
  if (is_wgs84) {
    cat(sprintf("✅ Successfully downloaded the full Tree Points dataset in WGS84 (EPSG:4326).\n"))
  } else {
    cat("⚠️ Tree Points loaded but CRS is not WGS84. Ensure st_transform(..., crs='WGS84').\n")
  }

  cat(sprintf("✅ Combined total: %s rows across %d saved page file(s).\n",
            format(n_rows, big.mark = ","), length(gjs)))
    if (!is.na(last_rows)) {
    if (grepl("_10000\\.geojson$", last_file)) {
      if (last_rows < 10000) {
        cat(sprintf("✅ Stop condition satisfied: last page has %,d rows (< 10,000).\n", last_rows))
      } else {
        cat("⚠️ Last page equals 10,000 rows; I may need additional pages. Re-run to continue.\n")
      }
    } else {
      cat(sprintf("ℹ️ Last page rows detected: %,d.\n", last_rows))
    }
  } else {
    cat("⚠️ Could not validate last page size. Files exist but unreadable; try re-running the download.\n")
  }
} else if (length(gjs) == 0) {
  cat("❌ Tree Points not found on disk. Run Task 2 (httr2 + paging + save pages).\n")
} else {
  cat("⚠️ Tree Points files exist, but the combined object is empty. Re-run the download and combine steps.\n")
}
...
- `health` – tree health rating (`Good`, `Fair`, `Poor`)\
- `status` – whether the tree is alive, dead, or stumped\
- `genusspecies` – scientific species name\
- `tpcondition` – overall condition assessment\
...
if (!is.na(n_rows) && n_rows > 0) {
cat("✅ Successfully downloaded the full Tree Points dataset in WGS84 (EPSG:4326).\n")
cat(sprintf("✅ Combined total: %s rows across %d saved page file(s).\n",
format(n_rows, big.mark=","), length(gjs)))
cat("✅ Data is aligned and ready for geospatial joins with council districts.\n")
} else {
cat("⚠️ Tree Points download incomplete — please re-run the task.\n")
}
...
#| code-fold: true
#| code-summary: "📦 Click to load cached district & tree data (faster re-runs)"
#| cache: true
#| echo: false
#| message: false
#| warning: false
...
cat("✅ Loaded cached Council Districts (WGS84, Release 25C)\n")
} else {
cat("⚠️ Council Districts not found — please re-run Task 1.\n")
}

if (file.exists("data/mp03/trees_wgs84_combined.rds")) {
trees <- readRDS("data/mp03/trees_wgs84_combined.rds")
cat("✅ Loaded cached Tree Points (WGS84, combined full dataset)\n")
} else {
cat("⚠️ Tree Points not found — please re-run Task 2.\n")
}
...
ggplot() +
  ...
  labs(
    ...
    caption  = "Sources: NYC Open Data (Forestry Tree Points) • NYC DCP (Council Districts)"
  ) +
...
#| code-fold: true
#| code-summary: "🧭 Click to see: spatial join (trees ↔ districts) + helpers"
#| cache: true
#| message: false
#| warning: false
...
# Spatial join: assign each tree to its district (POINTs first, REGIONS second → st_intersects)
...
cat("✅ Spatial join complete: trees linked to council districts.\n")
...
#| code-fold: true
#| code-summary: "🧩 District-level results (Q1–Q5)"
#| cache: true
#| message: false
#| warning: false
...
# Derive one area value per district in m² (NY State Plane EPSG:2263 → meters)
...
district_lookup_min <- districts |>
  ...
  transmute(
    ...
    Borough = dplyr::case_when(
      ...
      .data[[district_id_col]] %in% 33:48 ~ "Brooklyn",      # 33–48
      .data[[district_id_col]] %in% 49:51 ~ "Staten Island",# 49–51
      TRUE                               ~ NA_character_
    )
  )
...
cat(glue("Q1) Most trees → District {q1_top$district_id} ({q1_top$Borough}) ",
...
cat(glue("Q2) Highest density → District {q2_top$district_id} ({q2_top$Borough}) ",
...
cat(glue("Q3) Highest dead fraction → District {q3_top$district_id} ({q3_top$Borough}) ",
...
borough_map <- function(d) dplyr::case_when(
  ...
  d %in% 11:18 ~ "Bronx",
  d %in% 19:32 ~ "Queens",
  d %in% 33:48 ~ "Brooklyn",      # 33–48
  d %in% 49:51 ~ "Staten Island", # 49–51
  TRUE         ~ NA_character_
)
...
if (nrow(q4_manhattan_top)) {
  cat(glue("Q4) Most common species in Manhattan → {q4_manhattan_top$spc_common_} ",
...
if (nrow(closest_row)) {
  ...
  cat(glue("Q5) Closest tree to Baruch → Species: {ifelse(is.na(nearest_species), 'Unknown', nearest_species)} ",
...
**📘 Summary:** This section presents three concise tables summarizing the **Top NYC Council Districts**. In the urban forestry dataset. Each table integrates Borough information for better interpretability and spatial comparison.

```         
• Q1 shows the **Top districts with the highest total tree counts**, highlighting areas with the largest overall urban canopy.
• Q2 identifies the **Top districts by tree density** (trees per km²), showcasing regions with the most concentrated greenery.
• Q3 highlights the **Top districts with the highest proportion of dead trees**, indicating locations that may require greater maintenance attention.
```

```{r}
#| label: task4-tabular-outputs
#| code-fold: true
#| code-summary: "📄 Tables with Borough & Key Metrics (Top 10)"
#| cache: true
#| message: false
#| warning: false
...
kable(
  head(q1_table, 10),
  caption = "Q1: Districts by total tree count (Top 10) — with Borough"
)
...
kable(
  head(q2_table, 10),
  caption = paste0("Q2: Tree density (Top 10", scale_label, ") — with Borough")
)
...
kable(
  head(q3_table, 10),
  caption = "Q3: Dead tree fraction (Top 10) — with Borough"
)
```

------------------------------------------------------------------------

### 🌳 Task 5: NYC Parks Proposal — District 39 Tree Renewal Initiative

#### One-Page Summary — District 39 Tree Renewal Initiative
...
**Why District 39.** Despite NYC’s highest tree density, District 39 shows an elevated dead-tree share versus peer districts (e.g., 32, 35, 41), indicating urgent renewal needs.
...
**Delivery (FY2026).** Phase: stump removal → replacements → new plantings → pruning cycle; community volunteer events; public progress dashboard.

As the Council Member for **District 39 (Brooklyn)**, I propose the **District 39 Tree Renewal Initiative**, a neighborhood-focused effort to improve the balance between canopy coverage and tree health. Although District 39 has the **highest tree density in NYC (≈ 274 trees per km²)**, many trees are aging or at risk of decline. This project will focus on replacing old or dead trees and introducing new resilient species to ensure long-term canopy stability.
...
#| code-fold: true
#| code-summary: "🔢 Data check: D39 dead trees & stumps"
#| echo: false
#| message: false
#| warning: false
...
#### 🗺️ District 39 Tree Renewal Focus — Zoomed-In Brooklyn Map

This interactive visualization highlights the **tree renewal priority areas** in Brooklyn’s District 39. Clusters indicate concentrations of aging or dead trees identified for replacement, helping planners target renewal efforts efficiently.

```{r}
#| label: task5-map-d39-leaflet
#| fig-cap: "District 39 — interactive Leaflet with base-style switcher, alive/dead layers, clustered points, and optional heatmap."
#| code-fold: true
#| code-summary: "🧭 Interactive Leaflet (OSM default, pro styling, clustering)"
#| message: false
#| warning: false
...
> ⚙️ *Interactive map for District 39 (Brooklyn) showing tree conditions with toggles for Alive/Dead, an optional clustered view for performance, and a dead-tree heatmap layer. Use the base map switcher (OSM default, Carto Positron, Esri Gray) to present a clean, professional backdrop while exploring hotspots that motivate the renewal proposal.* ---

#### 📊 Comparative Analysis — District 39 vs Nearby Districts
...
#| fig-cap: "Dead Tree Fraction — District 39 vs nearby districts, with borough labels and interactive tooltips."
#| code-fold: true
#| code-summary: "🎯 Advanced dumbbell + borough labels + hover (Plotly, fixed title overlap)"
#| warning: false
#| message: false
...
fig <- ggplotly(p, tooltip = "text") %>%
  layout(
    ...
    title = list(
      text = "<b>Dead Tree Fraction — District 39 vs Nearby Districts</b><br><span style='font-size:13px;'>Dumbbell chart with borough labels; dashed line shows citywide median. Hover for details.</span>",
      x = 0.02, xanchor = "left"
    )
  ) %>%
  config(
...
### 🌳 Interactive Map — District 39 and Nearby Brooklyn Districts
...
#| code-fold: true
#| code-summary: "🧭 Leaflet: basemap+view fallback, layers, clusters, heatmap"
#| message: false
#| warning: false
...
# Quick sanity — will print in the render log
cat("Rows — districts:", nrow(dpoly), " | trees:", nrow(trees_cmp), "\n")
...
District 39 is already one of NYC’s greenest districts, but its high density and aging trees make it vulnerable to canopy loss. By replacing dead trees and replanting in gaps, the **District 39 Tree Renewal Initiative** will preserve Brooklyn’s tree coverage, reduce heat-island effects, and enhance the borough’s environmental resilience.

------------------------------------------------------------------------

### 🌿 Extra Credit #01: Interactive Version of the All-Trees Map

NYC’s tree dataset is extremely dense. This interactive map adds zoom/pan, popups, and clustering (plus an optional heatmap) so neighborhood-level patterns are legible and explorable—addressing the extra-credit goal.
...
#| code-fold: true
#| code-summary: "🌿 Interactive All-Trees Map (Leaflet + layers + clustering + heatmap)"
#| message: false
#| warning: false
...
m |>
  ...
  addControl("<b>💡 Tip:</b> Orange bubbles show clustered tree counts; enable Alive/Dead layers for detailed per-tree colors.",
             position = "bottomleft")
...
#| code-fold: true
#| code-summary: "🌳 EC#02 — SODA JSON (paged+cached) → clean → save → summaries + QC (all-in-one)"
#| message: false
#| warning: false
#| cache: true
#| results: "asis"
...
offset <- 0L
page   <- 1L
say("Starting SODA JSON download for %s …", dataset_id)
...
if (nrow(risk_by_boro_disp) > 0)
  knitr::kable(risk_by_boro_disp, caption = "Risk inspections by Borough × Rating (top 10)")
...
if (nrow(work_by_boro_disp) > 0)
  knitr::kable(work_by_boro_disp, caption = "Work orders by Borough × Type (top 10)")
...
#| code-fold: true
#| code-summary: "🌳 Work Orders by Borough — NYC Parks (EC #02)"
...
p <- plot_df2 %>%
  ...
  ggplot2::labs(
    ...
    subtitle = "Extra Credit #02 — downloaded politely via httr2 (JSON)",
...
print(p)
  ...
  knitr::asis_output(
    sprintf(
      ...
      ifelse(nrow(top_two) >= 2, top_two$borough[2], "—"),
      ifelse(nrow(top_two) >= 2, scales::comma(top_two$n_orders[2]), "—")
    )
  )

} else {
  knitr::kable(head(work_by_boro_tot),
               caption = "work_by_boro_tot is empty — run `p6_ec02_end_to_end` first.")
}
```
...
#| code-fold: true
#| code-summary: "🌳 Risk inspections by borough (stacked by rating)"
...
if (nrow(risk_tbl) == 0) {
  knitr::kable(
    ...
    caption = "risk_tbl is empty — run `p6_ec02_end_to_end` to download/clean the Risk dataset."
  )
} else {
  ...
  if (nrow(risk_by_boro) == 0) {
    knitr::kable(
      tibble(borough = character(), risk_rating = character(), n_inspections = integer()),
      caption = "risk_by_boro is empty — check the clean risk file."
    )
  } else {
    # 5) Plot
    ...
    p <- ggplot(risk_disp, aes(x = borough, y = n_inspections, fill = risk_label)) +
      ...
      labs(
        ...
        caption  = "Source: NYC Open Data (259a-b6s7) • Downloaded politely via httr2 (JSON)"
      ) +
    ...
    knitr::asis_output(sprintf(
      "\n**Result:** %s has the highest number of risk inspections (%s), meeting EC#02’s borough-level summary requirement.\n",
...
Available columns → ",
    ...
    "Non-missing counts → borough:", nonmiss["borough"],
    ", borocode:", nonmiss["borocode"],
    ", boroughcode:", nonmiss["boroughcode"],
    ", boro_code:", nonmiss["boro_code"],
    ", zipcode:", nonmiss["zipcode"], "\n\n",
    "Conclusion: No usable borough information found → aggregation drops all rows, so the risk chart is blank.\n", sep = "")
```
#### 📄 Appendix: ZIP-level summary
...
if (exists("work_by_zip") && is.data.frame(work_by_zip) && nrow(work_by_zip) > 0) {
  ...
  cat("**Table – Top 25 ZIP codes by total forestry work orders (NYC Parks)**  \n")
  knitr::kable(zip_tot, align = "c")

} else {
  cat(
"> ⚠️ **Data Note — ZIP-Level Summary Unavailable**  \n",
"> The object `work_by_zip` is currently empty.  \n",
"> This usually occurs when the EC#02 end-to-end pipeline hasn’t been re-run recently  \n",
...
```{r}
#| code-fold: true
#| code-summary: "📝 Automated summary with data check"
#| message: false
#| warning: false
#| results: asis
#| dependson: p6_ec02_end_to_end
...
if (
  ...
  cat(glue(
    ...
    "(2017–present). The highest workload is observed in **{top_boro_orders$borough}** ",
  ...
  cat(
"> ⚠️ **Data Note — Summary Unavailable**  \n",
...
> **Sources.** NYC Open Data — *Forestry Risk Assessments* (dataset ID `259a-b6s7`) and *Forestry Work Orders* (dataset ID `bdjm-n7q4`). Downloaded politely with `httr2` using SODA JSON (paged) and cached locally.


### ✅ Submission Checklist
- Render to `docs/mp03.html` (GitHub Pages).  
- “Print to PDF” and upload to Brightspace (Optional).  